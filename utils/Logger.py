...
class Logger():
    ...
    # Add
    def save_single_best_acc(self, val_res=None, test_res=None, val_best=None, idx_val_best=None, test_domain=None):
        # idx_val_best = val_res.argmax()
        idx_test_best = test_res.argmax()
        print()
        print('*' * 30)
        # print("Best val %g, corresponding test %g - best test: %g" % (val_res.max(), test_res[idx_val_best], test_res.max()))
        print(f"Best val {val_best}, corresponding {test_domain} test {test_res[idx_val_best]}")
